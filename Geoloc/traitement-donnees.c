...
void LambertToGPS(double XLAMB, double YLAMB) {
  ...
  float gl1    = log( tan( M_PI / 4 + phi1 / 2) * pow( (1 - E * sin( phi1 ) ) / ( 1 + E * sin( phi1 ) ), E / 2) );
  float gl2    = log( tan( M_PI / 4 + phi2 / 2) * pow( (1 - E * sin( phi2 ) ) / ( 1 + E * sin( phi2 ) ), E / 2) );
